...
export const GameRow = memo(
  function GameRow({
    ...
    onLaunchEnd: (
      gameKey: string,
      cooldownMs?: number,
      options?: { primaryLaunch?: boolean }
    ) => void
    onRunningStateRefresh: () => Promise<void>
    onToggleEditor: (gameKey: string) => void
    ...
    onCloseEditor: (gameKey: string) => void
            ...
            {isActive && (
              <div className="pb-4">
                <ProfileEditor
                  ...
                  onClose={() => onCloseEditor(game.key)}
                  onCreateProfile={() =>
  ...
  (prev, next) => {
    if (
      prev.game.key !== next.game.key ||
      prev.isActive !== next.isActive ||
      prev.isRunning !== next.isRunning ||
      prev.isGameRunning !== next.isGameRunning ||
      prev.gameStatusWarning !== next.gameStatusWarning ||
      prev.gameStatusDismissPath !== next.gameStatusDismissPath ||
      prev.gameStatusTracked !== next.gameStatusTracked ||
      prev.gameIconUrl !== next.gameIconUrl ||
      prev.isDimmed !== next.isDimmed ||
      prev.isLaunching !== next.isLaunching ||
      prev.isLaunchBlocked !== next.isLaunchBlocked ||
      prev.cacheInitialized !== next.cacheInitialized ||
      prev.onLaunchStart !== next.onLaunchStart ||
      prev.onLaunchEnd !== next.onLaunchEnd ||
      prev.onRunningStateRefresh !== next.onRunningStateRefresh ||
      prev.onToggleEditor !== next.onToggleEditor ||
      prev.onCloseEditor !== next.onCloseEditor
    ) {
      return false
    }

    if (prev.runningAppIcons.length !== next.runningAppIcons.length) {
      return false
    }

    for (let i = 0; i < prev.runningAppIcons.length; i++) {
      const prevIcon = prev.runningAppIcons[i]
      const nextIcon = next.runningAppIcons[i]
      if (
        prevIcon.icon !== nextIcon.icon ||
        prevIcon.name !== nextIcon.name ||
        prevIcon.path !== nextIcon.path ||
        prevIcon.gameKey !== nextIcon.gameKey ||
        prevIcon.warning !== nextIcon.warning ||
        prevIcon.elevated !== nextIcon.elevated ||
        prevIcon.tracked !== nextIcon.tracked
      ) {
        return false
      }
    }
